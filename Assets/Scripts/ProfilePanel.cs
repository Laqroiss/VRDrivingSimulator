...
public class ProfilePanel : MonoBehaviour
{
    ...
    public void Refresh()
    {
        ...
        if (_summary != null) _summary.text = "LoadingвЂ¦";
        StopAllCoroutines();
    ...
    IEnumerator FetchAttempts(string studentName)
    {
        ...
        if (_summary != null)
            _summary.text = best == int.MaxValue
                ? $"Attempts: {total}   вЂў   Passed: {passed}"
                : $"Attempts: {total}   вЂў   Passed: {passed}   вЂў   Best: {best} pts";
    ...
    void AddRow(Attempt a)
    {
        ...
        if (!a.hasReplay)                       // no replay recorded вЂ” nothing to play
        {
    ...
    static string FormatDate(string iso)
    {
        ...
        return string.IsNullOrEmpty(iso) ? "вЂ”" : iso;
    }
}

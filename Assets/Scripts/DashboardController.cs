...
/// <summary>
/// Handles only the dashboard visuals:
/// 1. Speedometer, tachometer, RPM
/// 2. Turn-signal stalk animation - reads state from CarIndicators
/// 3. Extra turn-signal UI icons (if you want a second set besides those in CarIndicators)
///
/// Turn-signal control and steering auto-cancel live in CarIndicators, not here.
/// </summary>
public class DashboardController : MonoBehaviour
{
    [Header("Tachometer / speedometer")]
    ...
    [Header("Gear (P R N D + gear number)")]
    public TextMeshProUGUI gearText;
    [Tooltip("Show the gear number next to D (D1, D2, ...)")]
    ...
    [Header("Extra turn-signal UI icons (optional)")]
    [Tooltip("Leave empty if the icons are already set up in CarIndicators")]
    ...
    [Header("Turn-signal stalk animation")]
    ...
    void Update()
    {
        ...
        if (carScript.rb == null) return; // car not initialized yet (menu)
    ...
    // Syncs the extra icons (if any) with CarIndicators

    ...
    IEnumerator ReplayRoutine(ReplaySession session)
    {
        ...
        while (_replaying)
        {
            ...
            if (replayTimeLabel != null)
            {
                ...
                replayTimeLabel.text = $"{secs:F1}s / {duration:F1}s  вЂў  {f.speed:F0} km/h  D{f.gear}  {f.rpm:F0} RPM";
            }
    ...
    void CreateGhost()
    {
        ...
        if (car.wheels != null)
        {
            for (int i = 0; i < car.wheels.Length; i++)
            {
                ...
                {
                    Debug.LogWarning($"[Replay] wheels[{i}].wheelObject == null вЂ”  ");
                ...
                for (int j = 0; j < i; j++)
                {
                    if (car.wheels[j].wheelObject == w.wheelObject)
                    {
                        ...
                        Debug.LogWarning($"[Replay] wheels[{i}]  wheels[{j}] вЂ”   {w.wheelObject.name}");
                        duplicate = true;
                        break;
                    }
                }

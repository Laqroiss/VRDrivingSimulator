  ...
  return (
    <div style={{ paddingBottom: 60 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 32 }}>
        <div>
          ...
          <p style={{ color: 'var(--muted)', marginTop: 4 }}>
            ...
            : {Math.floor(a.examDuration/60)}:{String(Math.round(a.examDuration%60)).padStart(2,'0')}
          </p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
        {/*   */}
        <div className="card">
          <h2 style={{ fontSize: 14, fontWeight: 600, marginBottom: 14, color: 'var(--muted)' }}></h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {(a.exerciseStatuses || []).map((s, i) => (
              <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span>{EXERCISE_NAMES[i + 1] || `. ${i + 1}`}</span>
      ...
      {a.track && a.track.length > 0 && (
        <div className="card" style={{ marginBottom: 20 }}>
          <h2 style={{ fontSize: 14, fontWeight: 600, marginBottom: 16, color: 'var(--muted)' }}>2D </h2>
          <ReplayViewer
            track={a.track}
      ...
      {a.track && a.track.length > 0 && (
        <div className="card">
          <h2 style={{ fontSize: 14, fontWeight: 600, marginBottom: 16, color: 'var(--muted)' }}>  RPM</h2>

...
export default function ReplayViewer({ track, penalties, lightEvents = [], lightPositions = [] }) {
  ...
  const [activeLight, setActiveLight] = useState(null) // id 
  ...
  const seekToError = (p) => {
    ...
    if (p.t > 0) {
      ...
      p.x != null && p.z != null &&
      Math.hypot(p.x - startX, p.z - startZ) > 1  //    
    ) {
      // :   
      track.forEach((pt, i) => {
      ...
      <div style={{ position: 'relative', display: 'inline-block', maxWidth: '100%' }}>
        ...
        {lightPositions.length === 0 && (
          <div style={{
            ...
            No traffic-light data — retake the exam
          </div>
        )}
        ...
        <div style={{ marginTop: 16 }}>
          <p style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 8 }}>
            {penalties.some(p => p.t > 0)
              ...
              : ' No timing data for errors — retake the exam to enable seeking'}
          </p>
